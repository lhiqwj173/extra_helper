...
namespace extra {
	namespace lzma {
		...
		MYLIB_API void decompress_mem_thread(const char* data, std::function<void(std::unique_ptr<uint8_t[]>, uint32_t)> cb);
		MYLIB_API void decompress_mem_thread(const uint8_t* data, std::function<void(std::unique_ptr<uint8_t[]>, uint32_t)> cb);
